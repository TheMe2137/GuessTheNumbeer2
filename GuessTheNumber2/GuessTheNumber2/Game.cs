...
public class Game   //class having game oriented objects/methods
{
    ...
    public static Score startGame(int maxNumber, string difficultyName, bool newGamePlus)      //method that contains main game thingy
    {
        ...
        {
            Gui.PrintText(Language.Get("lmtd_q"));
            ...
            {
                Gui.PrintText(Language.Get("lmtd_q_2"));
        ...
        {
            Gui.PrintText(Language.Get("guess"));
            int guessedNumber;
            ...
            while (guessedNumber != number && limitedAttempts > 0)
            {
                ...
                if (guessedNumber < number)     //when you hit smaller
                {
                    ...
                    Gui.PrintText(Language.Get("too_little") + randomPhrases + Language.Get("current_attempt") + attempts + Language.Get("attempts_remain") + limitedAttempts);
                    attempts++;
                    limitedAttempts--;
                ...
                {
                    string randomPhrases = Phrases[Rnd.Next(0, Phrases.Length)];
                    Gui.PrintText(Language.Get("Too_high")+randomPhrases + Language.Get("current_attempt") + attempts + Language.Get("attempts_remain")+ limitedAttempts);
                    attempts++;
                    limitedAttempts--;
                    Random phraseNumber = new Random();
            ...
            {
                Gui.PrintText(Language.Get("fail"));
                Console.ReadKey();
                Console.Clear();
                return null;
            }
            ...
            Gui.PrintText(Language.Get("win"));
        ...
        else        //when u dont have limited attempts
        {
            ...
            while (guessedNumber != secretNumber)
            {
                ...
                {
                    string randomPhrases = Phrases[Rnd.Next(0, Phrases.Length)];
                    Gui.PrintText(Language.Get("too_little")+randomPhrases + Language.Get("current_attempt") + attempts);
                    attempts++;
                    Random phraseNumber = new Random();
                    while (!int.TryParse(Console.ReadLine(), out guessedNumber) || guessedNumber < 0 || guessedNumber >= maxNumber+1)
                    {
                ...
                {
                    string randomPhrases = Phrases[Rnd.Next(0, Phrases.Length)];
                    Gui.PrintText(Language.Get("too_high") +randomPhrases + Language.Get("current_attempt") + attempts);
                    Random phraseNumber = new Random();
                    attempts++;
                    while (!int.TryParse(Console.ReadLine(), out guessedNumber) || guessedNumber < 0 || guessedNumber >= maxNumber+1)
                    {
            ...
            Gui.PrintText(Language.Get("win"));     //win
            string playerName = Console.ReadLine();
            Console.Clear();
