...
{
    static List<Score> scores = new List<Score>();  //list holding a score using score contructor from score.cs
    ...
    {
        Console.Clear();
        Gui.PrintText(Language.Get("leaderboardshow"));
    ...
    public static void ShowByDifficulty(string difficulty)      //method printing out leaderboard  for specific difficulty
    {
        ...
        Console.WriteLine(Language.Get("leaderstart"));


        if (scores.Count == 0)      //checks if scores has any counts
        {
            Gui.PrintText(Language.Get("noscores"));
        }
        else
        {
            foreach (var score in filteredScores)   //loop printing out rows from leaderboard
            {
                if (score.isNewGamePlus)
                {
                    ngPlusMark = " NG+";
                }
                Console.WriteLine($"{position}. {score.PlayerName} - {score.Attempts} {Language.Get("attempts")} {Language.Get("time")}  {score.TimeSeconds}s{ngPlusMark}");
                position++;
            }
        }


        Console.WriteLine(Language.Get("return"));
        Console.ReadKey();
    }
